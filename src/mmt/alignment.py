...
def make_alignment(source_indexes, target_indexes, attention_matrix, prefix_lang=None, symmetrize=sym_intersect):
    # dimensions truncated to the size of trg_sub_tokens and src_sub_tokens

    # fairseq_0.7: attention_matrix has size (len(source_indexes)+1, len(target_indexes)+1)
    # fairseq_0.10: attention_matrix has size (max_len_source+1, max_len_target+1)
    #    where max_len_source is the maximum source_length among all sentences in a batch
    #    where max_len_target is the maximum target_length among all sentences in a batch
    if is_fairseq_0_10():
        reduced_attention_matrix = attention_matrix[-len(source_indexes)-1:-1, -len(target_indexes)-1:-1]
    else:
